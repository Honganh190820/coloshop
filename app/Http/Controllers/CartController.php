...
class CartController extends GeneralController
{
    ...
    public function postCheckout(Request $request)
    {
        $request->validate([
            ...
            'fullname.required'=> 'Bạn vui lòng nhập tên người nhận',
            'phone.required'=> 'Bạn vui lòng nhập số điện thoại',
            'email.required'=> 'Bạn vui lòng nhập email để nhận hóa đơn',
            'address.required'=> 'Bạn vui lòng nhập địa chỉ nhận hàng',
        ]);
        ...
        if ($order->save()) {
            ...
            $order->code = 'DH-' . time() . '-' . date('d') . date('m') . date('Y');
            $order->save();
            $orderDetail = []; //tạo email
            $orderDetails = [];
